...
// Utility para gravação de logs do sistema
function registrarLog(usuario, acao, detalhes = '') {
    ...
    const query = `INSERT INTO logs_sistema (usuario, acao, detalhes, data_hora) VALUES (?, ?, ?, ?)`;
    db.run(query, [usuario || 'Sistema', acao, detalhes, dataHoraBrasil], (err) => {
...
async function imprimirComandaAuto(pedido) {
    ...
    try {
        ...
        stream.on('finish', async () => {
            try {
                // Imprime usando a impressora padrão configurada no Windows
                await ptp.print(filePath);
                console.log(`🖨️ Comanda do Pedido #${pedido.id} impressa com sucesso!`);
            } catch (err) {
...
db.serialize(() => {
    ...
    db.get("SELECT COUNT(*) as count FROM adicionais", (err, row) => {
        if (row && row.count === 0) {
            const stmt = db.prepare("INSERT INTO adicionais (nome, preco, disponivel) VALUES (?, ?, 1)");
...
app.post('/api/login', (req, res) => {
    ...
    db.get("SELECT * FROM usuarios WHERE login = ? AND senha = ?", [login, senha], (err, row) => {
        ...
        if (!row) {
            registrarLog('Visitante', 'LOGIN_FALHA', `Tentativa de login malsucedida para usuário: "${login}"`);
            return res.status(401).json({ success: false, message: 'Usuário ou senha incorretos.' });
        }

        req.session.user = { login: row.login, role: row.role };
        registrarLog(row.login, 'LOGIN', `Sessão iniciada com sucesso [Cargo: ${row.role}]`);
...
app.post('/api/logout', (req, res) => {
    ...
    registrarLog(usuario, 'LOGOUT', 'Sessão encerrada');
    req.session.destroy();
...
app.post('/api/admin/usuarios', adminOnly, (req, res) => {
    ...
    db.run("INSERT INTO usuarios (login, senha, role) VALUES (?, ?, ?)", [login, senha, userRole], function (err) {
        ...
        registrarLog(adminUser, 'CRIAR_USUARIO', `Cadastrado novo usuário "${login}" [Role: ${userRole}]`);
...
app.put('/api/admin/usuarios/:id', adminOnly, (req, res) => {
    ...
    if (senha && senha.trim() !== '') {
        ...
        db.run(query, [login, senha, userRole, id], function (err) {
            ...
            registrarLog(adminUser, 'EDITAR_USUARIO', `Atualizado usuário ID #${id} (${login}) [Role: ${userRole}, Senha Alterada]`);
            res.json({ success: true });
        });
    } else {
        ...
        db.run(query, [login, userRole, id], function (err) {
            if (err) return res.status(500).json({ error: 'Erro ao atualizar usuário ou login já existe.' });
            registrarLog(adminUser, 'EDITAR_USUARIO', `Atualizado usuário ID #${id} (${login}) [Role: ${userRole}]`);
            res.json({ success: true });
        });
    }
});
...
app.delete('/api/admin/usuarios/:id', adminOnly, (req, res) => {
    ...
    db.run("DELETE FROM usuarios WHERE id = ?", [req.params.id], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'EXCLUIR_USUARIO', `Usuário ID #${req.params.id} excluído do sistema`);
        res.json({ success: true });
    });
});
...
app.post('/api/pedidos', authRequired, (req, res) => {
    ...
    db.run(query, [cliente, tamanho, acompStr, adicStr, observacoes, total, dataHoraBrasil], function (err) {
        ...
        registrarLog(usuarioAtivo, 'NOVO_PEDIDO', `Pedido #${this.lastID} enviado para a cozinha (Cliente: ${cliente}, Item: ${tamanho}, Total: R$ ${parseFloat(total).toFixed(2)})`);
        res.json({ success: true, id: this.lastID });
    });
});
...
app.put('/api/pedidos/:id/concluir', staffOnly, (req, res) => {
    ...
    db.run("UPDATE pedidos SET status = 'PRONTO', data_conclusao = ? WHERE id = ?", [dataConclusaoLocal, id], function (err) {
        ...
        registrarLog(usuarioAtivo, 'PEDIDO_CONCLUIDO', `Pedido #${id} finalizado na cozinha e marcado como pronto`);
        res.json({ success: true });
    });
});
    ...
    db.serialize(() => {
        db.run("DELETE FROM pedidos", (err) => {
            ...
            db.run("DELETE FROM sqlite_sequence WHERE name='pedidos'", (err2) => {
                ...
                registrarLog(adminUser, 'LIMPAR_HISTORICO', 'Todo o histórico de pedidos foi excluído e o contador de ID foi resetado');
...
app.post('/api/caixa/fechar', adminOnly, (req, res) => {
    ...
    db.run(query, [dataAtual, fundo_troco || 0, dinheiro || 0, cartao || 0, pix || 0, ifood || 0, sangria || 0, total_gaveta || 0, faturamento_total || 0, responsavel, dataHoraBrasil], function(err) {
        ...
        registrarLog(responsavel, 'FECHAR_CAIXA', `Fechamento de caixa registrado para o dia ${dataAtual} - Total: R$ ${parseFloat(faturamento_total || 0).toFixed(2)}`);
        res.json({ success: true, id: this.lastID });
    });
});
...
app.delete('/api/admin/relatorio/reset', adminOnly, (req, res) => {
    ...
    db.run("DELETE FROM pedidos", [], function (err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'RESET_RELATORIO', 'Todos os dados de relatórios e vendas do sistema foram resetados');
    ...
    db.serialize(() => {
        db.run("DELETE FROM pedidos", (err) => {
            ...
            db.run("DELETE FROM sqlite_sequence WHERE name='pedidos'", (err2) => {
                ...
                registrarLog(adminUser, 'RESET_IDS_PEDIDOS', 'Pedidos apagados e contador de ID reiniciado para 1');
...
app.post('/api/tamanhos', adminOnly, (req, res) => {
    ...
    db.run("INSERT INTO tamanhos (nome, preco, disponivel) VALUES (?, ?, ?)", [nome, preco, statusDisp], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'CRIAR_TAMANHO', `Cadastrado novo tamanho/copo "${nome}" - Preço: R$ ${parseFloat(preco).toFixed(2)}`);
        res.json({ success: true, id: this.lastID });
    });
});
...
app.put('/api/tamanhos/:id', adminOnly, (req, res) => {
    ...
    if (disponivel !== undefined && nome === undefined && preco === undefined) {
        db.run("UPDATE tamanhos SET disponivel = ? WHERE id = ?", [disponivel ? 1 : 0, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'BLOQUEIO_ESTOQUE', `Disponibilidade do Tamanho ID #${req.params.id} alterada para [${disponivel ? 'Disponível' : 'Indisponível'}]`);
            res.json({ success: true });
        });
    } else if (nome) {
        const statusDisp = disponivel !== undefined ? (disponivel ? 1 : 0) : 1;
        db.run("UPDATE tamanhos SET nome = ?, preco = ?, disponivel = ? WHERE id = ?", [nome, preco, statusDisp, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'EDITAR_TAMANHO', `Tamanho ID #${req.params.id} alterado para "${nome}" - R$ ${parseFloat(preco).toFixed(2)}`);
            res.json({ success: true });
        });
    } else {
        db.run("UPDATE tamanhos SET preco = ? WHERE id = ?", [preco, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'EDITAR_TAMANHO', `Preço do Tamanho ID #${req.params.id} alterado para R$ ${parseFloat(preco).toFixed(2)}`);
            res.json({ success: true });
        });
    }
});
...
app.delete('/api/tamanhos/:id', adminOnly, (req, res) => {
    ...
    db.run("DELETE FROM tamanhos WHERE id = ?", [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'EXCLUIR_TAMANHO', `Tamanho/Copo ID #${req.params.id} excluído do cardápio`);
        res.json({ success: true });
    });
});
...
app.post('/api/acompanhamentos', adminOnly, (req, res) => {
    ...
    db.run("INSERT INTO acompanhamentos (nome, disponivel) VALUES (?, ?)", [nome, statusDisp], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'CRIAR_ACOMPANHAMENTO', `Cadastrado novo acompanhamento "${nome}"`);
        res.json({ success: true, id: this.lastID });
    });
});
...
app.put('/api/acompanhamentos/:id', adminOnly, (req, res) => {
    ...
    if (disponivel !== undefined && nome === undefined) {
        db.run("UPDATE acompanhamentos SET disponivel = ? WHERE id = ?", [disponivel ? 1 : 0, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'BLOQUEIO_ESTOQUE', `Disponibilidade do Acompanhamento ID #${req.params.id} alterada para [${disponivel ? 'Disponível' : 'Indisponível'}]`);
            res.json({ success: true });
        });
    } else {
        db.run("UPDATE acompanhamentos SET nome = ? WHERE id = ?", [nome, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'EDITAR_ACOMPANHAMENTO', `Acompanhamento ID #${req.params.id} renomeado para "${nome}"`);
            res.json({ success: true });
        });
    }
});
...
app.delete('/api/acompanhamentos/:id', adminOnly, (req, res) => {
    ...
    db.run("DELETE FROM acompanhamentos WHERE id = ?", [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'EXCLUIR_ACOMPANHAMENTO', `Acompanhamento ID #${req.params.id} excluído do cardápio`);
        res.json({ success: true });
    });
});
...
app.post('/api/adicionais', adminOnly, (req, res) => {
    ...
    db.run("INSERT INTO adicionais (nome, preco, disponivel) VALUES (?, ?, ?)", [nome, preco, statusDisp], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'CRIAR_ADICIONAL', `Cadastrado novo adicional "${nome}" - Preço: R$ ${parseFloat(preco).toFixed(2)}`);
        res.json({ success: true, id: this.lastID });
    });
});
...
app.put('/api/adicionais/:id', adminOnly, (req, res) => {
    ...
    if (disponivel !== undefined && nome === undefined && preco === undefined) {
        db.run("UPDATE adicionais SET disponivel = ? WHERE id = ?", [disponivel ? 1 : 0, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'BLOQUEIO_ESTOQUE', `Disponibilidade do Adicional ID #${req.params.id} alterada para [${disponivel ? 'Disponível' : 'Indisponível'}]`);
            res.json({ success: true });
        });
    } else {
        const statusDisp = disponivel !== undefined ? (disponivel ? 1 : 0) : 1;
        db.run("UPDATE adicionais SET nome = ?, preco = ?, disponivel = ? WHERE id = ?", [nome, preco, statusDisp, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            registrarLog(adminUser, 'EDITAR_ADICIONAL', `Adicional ID #${req.params.id} alterado para "${nome}" - R$ ${parseFloat(preco).toFixed(2)}`);
            res.json({ success: true });
        });
    }
});
...
app.delete('/api/adicionais/:id', adminOnly, (req, res) => {
    ...
    db.run("DELETE FROM adicionais WHERE id = ?", [req.params.id], function(err) {
        if (err) return res.status(500).json({ error: err.message });
        registrarLog(adminUser, 'EXCLUIR_ADICIONAL', `Adicional ID #${req.params.id} excluído do cardápio`);
        res.json({ success: true });
    });
});
...
app.get('/api/admin/logs', adminOnly, (req, res) => {
    db.all("SELECT * FROM logs_sistema ORDER BY id DESC LIMIT 200", [], (err, rows) => {
        if (err) return res.status(500).json({ error: err.message });
        res.json(rows);
    });
});
